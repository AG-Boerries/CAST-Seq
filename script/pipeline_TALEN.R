...
if(filtName != ""){
  ...
  ################     CHR PLOT (CIRCLIZE)    ################ 
	...
	tryCatch(
    	{
 	circlizePipelineTALEN(siteFile = file.path(guideD, paste0(sampleName, "_w", w, "_FINAL.xlsx")),
...
coverage_talen(inputFile = file.path(guideD, paste0(sampleName, "_w", w, "_FINAL.xlsx")),
                bamFile = file.path(sampleD, "results", "fastq_aln", paste0(sampleName, "_AlignmentSort.bam")),
                window.size = 100,
