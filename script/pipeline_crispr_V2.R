...
runPipelineCRISPR <- function()
{
  ...
  if(file.exists(file.path(qcDir, "alignment_QC.xlsx"))){
    ...
    countReads(fastqFolder = c(fastqD.treated.unique, fastqD.untreated.unique),
               sampleFolder = sampleD,
    ...
    lapply(1:length(samples.treated.unique), function(i){
      sample.current <- samples.treated.unique[i]
      label.current <- labels.treated.unique[i]
      
      plotReadLength(fastqGZ = file.path(sampleD, label.current, "fastq_aln", paste0(sample.current, "_trim3_soft.fastq.gz")),
                     outName = file.path(qcDir, paste0(sample.current, "_trim3_soft")))
    })
    
    lapply(1:length(samples.untreated.unique), function(i){
      sample.current <- samples.untreated.unique[i]
      label.current <- labels.untreated.unique[i]
  ...
  lapply(1:length(samples.treated), function(i){
    ...
    #fastqD <- fastqD.treated.unique[i]
  ...
  # EXTEND BIN FOR ON-TARGET SITE
  extend_bin(inputFile = file.path(ovlD, paste0(projectName, "_BIN.xlsx")),
             outputFile = file.path(ovlD, paste0(projectName, "_BIN.xlsx")),
             otsF = otsBed,
             ext.size = 200)
  ...
  dir.create(file.path(covD, "ONtarget"), showWarnings = FALSE)
  ...
  ONreadout(bamFile = bamFiles,
            ...
            outDir = file.path(covD, "ONtarget"))
  ...
  # UN-treated
  bamFiles.ut <- sapply(1:length(labels.untreated.unique), function(i)
    file.path(sampleD, labels.untreated.unique[i], "fastq_aln", paste0(samples.untreated.unique[i], "_AlignmentSort.bam")))
  
  ONreadout(bamFile = bamFiles.ut,
            otsFile = otsBed,
            gRNA.orientation = bpOrient,
            window.size = 5000,
            sampleName = paste0(projectName, "_UT"),
            outDir = file.path(covD, "ONtarget"))
  ...
  getGuideAlignment(inputF = file.path(ovlD, paste0(projectName, "_BIN.xlsx")),
  ...
  ################     DEFINE GROUPS    ################ 
  print("################     DEFINE GROUPS    ################ ")
  assignGroups(file.path(ovlD, paste0(projectName, "_FLANK.xlsx")),
               file.path(randomD, paste0(randomName, "_FLANK.xlsx")),
  ...
  guidePlot(file.path(ovlD, paste0(projectName, "_GROUP.xlsx")),
            file.path(ovlD, paste0(projectName, "_ALN_RAW_heatmap.pdf")),
            score = NULL, pv = NULL, ref = refSeq,
            OMTonly = FALSE, reorder = "hits",
            nbOvl = NULL, nbSignif = NULL)# ALL
  
  guidePlot(file.path(ovlD, paste0(projectName, "_GROUP.xlsx")),
            file.path(ovlD, paste0(projectName, "_ALN_heatmap.pdf")),
            score = NULL, pv = NULL, ref = refSeq,
            OMTonly = FALSE, reorder = "hits",
            nbOvl = nb.ovl, nbSignif = nb.signif)# SELECTED ONLY
  
  # OMT ONLY
  guidePlot(file.path(ovlD, paste0(projectName, "_GROUP.xlsx")),
            file.path(ovlD, paste0(projectName, "_ALN_OMT_heatmap.pdf")),
            score = NULL, pv = NULL, ref = refSeq,
            OMTonly = TRUE, reorder = "hits",
            nbOvl = nb.ovl, nbSignif = nb.signif)# SELECTED ONLY
  
  # LOGO PLOT
  logoPlot(file.path(ovlD, paste0(projectName, "_GROUP.xlsx")),
           file.path(ovlD, paste0(projectName, "_ALN_RAW_logo.pdf")),
           score = NULL, pv = NULL, ref = refSeq,
           nbOvl = NULL, nbSignif = NULL)# ALL
  
  logoPlot(file.path(ovlD, paste0(projectName, "_GROUP.xlsx")),
           file.path(ovlD, paste0(projectName, "_ALN_logo.pdf")),
           score = NULL, pv = NULL, ref = refSeq,
           nbOvl = nb.ovl, nbSignif = nb.signif)# SELECTED ONLY
  ...
  coverage_cleavage_crispr(inputFile = file.path(ovlD, paste0(projectName, "_FINAL.xlsx")),
                    covDir = covD,
                    bin.size = 100)
  ...
  if(rmTMP){
    ...
    mypattern <- c("_assembled.fastq.gz$", "_Filt2.fastq.gz$", "_Filt3_neg.fastq.gz$",
                   ...
                   "_pos.fastq.gz$", "_trim1.fastq.gz$", "_trim2.fastq.gz$", "_trim3.fastq.gz$", "_trim3_soft.fastq.gz$",
                   "_unassembled.R1.fastq.gz$", "_unassembled.R2.fastq.gz$",
                   "_merged.fastq.gz$",
                   "_SELECTED.xlsx$", "_BIN.xlsx$", "_BH.xlsx$", "_ALN.xlsx$", "_ALN_PV.xlsx$",
