...
runPipelineTALEN <- function()
{
  ...
  getGuideAlignmentDual(inputF = file.path(ovlD, paste0(projectName, "_BIN.xlsx")),
  ...
  print("################     ASSIGN PVALUE    ################")
  assignPV(file.path(ovlD, paste0(projectName, "_ALN.xlsx")),
           file.path(randomD, paste0(randomName, "_ALN.xlsx")),
           file.path(ovlD, paste0(projectName, "_ALN_PV.xlsx")),
           nbTop = 3,
           plot = TRUE)
  ...
  # REAL SEQUENCES
  if(is.null(flank1.sq)){
    addFlanking(file.path(ovlD, paste0(projectName, "_ALN_PV.xlsx")),
                file.path(ovlD, paste0(projectName, "_FLANK.xlsx")),
                otsBed, flankingSize)
  }else{
    addFlankingFromSq(file.path(ovlD, paste0(projectName, "_ALN_PV.xlsx")),
                      file.path(ovlD, paste0(projectName, "_FLANK.xlsx")),
                      flank1.sq, flank2.sq)
  }
  ...
  ################     DEFINE GROUPS    ################ 
  print("################     DEFINE GROUPS    ################ ")
  assignGroupsTALEN(file.path(ovlD, paste0(projectName, "_FLANK.xlsx")),
                    file.path(randomD, paste0(randomName, "_FLANK.xlsx")),
  ...
  # Annotation
  annotateGeneTALEN(file.path(ovlD, paste0(projectName, "_GROUP.xlsx")),
  ...
  addGenesTALEN(file.path(ovlD, paste0(projectName, "_GENES.xlsx")),
  ...
  coverage_cleavage_talen(inputFile = file.path(ovlD, paste0(projectName, "_FINAL.xlsx")),
                          covDir = covD,
                          bin.size = 100)
  ...
  circlizePipelineTALEN(siteFile = file.path(ovlD, paste0(projectName, "_FINAL.xlsx")),
  ...
  circlizePipelineTALENNOZOOM(siteFile = file.path(ovlD, paste0(projectName, "_FINAL.xlsx")),
                              label = FALSE,
                              PV.cutoff = NULL,
                              bestScore.cutoff = NULL, bestFlank.cutoff = 25,
                              showNBS = TRUE,
